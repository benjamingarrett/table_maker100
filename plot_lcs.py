...
# plot 6 lcs traceback lru to find cutoff and make conjecture
# taking from above to reduce to cleaner version
if plot_6:
  results_path = './lcs_binary_search'
  f = lambda x: x if len(x) > 0 else '0'
  d = os.listdir(results_path)
  ...
  for new_field in d:
    ...
    if len(y) > 0:
      max_prod = max(prod)
      worst_x = [xx for j,xx in enumerate(x) if prod[j]==max_prod][0]
      min_prod = min(prod)
      best_x = [xx for j,xx in enumerate(x) if prod[j]==min_prod][0]
    else:
      max_prod = 0
      worst_x = 0
      min_prod = 0
    ...
    if True:
      cutoff = n**2-n
      #cutoff = max(x)*0.9 if len(x) > 0 else n
      ...
      prod = [y[j]*xx for j,xx in enumerate(x)]
      c1 = min(prod)/(n**4) if len(prod) > 0 else 1
      c2 = max(prod)/(n**4) if len(prod) > 0 else 1
      #print('for n = {} c1 = {} c2 = {}'.format(n,c1,c2))
      print('for n = {} c1 = {} c2 = {} worst_x = {} max_prod = {} best_x = {} min_prod = {} '.format(n,c1,c2,worst_x,max_prod,best_x,min_prod))
      lower = [c1*n for xx in enumerate(x)]
      upper = [c2*n for xx in enumerate(x)]
      #ax.plot(x,lower,'--')
      #ax.plot(x,upper,':')
    label_ = 'n='+str(n)
    ax.plot(x,prod,label=label_)
  plt.xlabel('cache size')
  plt.ylabel('(cache size)x(cache misses)')
  ax.legend(loc='upper right')
  t = 'LCS traceback, LRU, input type 0^k 1^k, product of cache size and cache misses as a function of cache size, input sizes 4 - 1024'
  t += '\ncutoff at cache size = n^2-n'
  plt.title(t)
  plt.show()
